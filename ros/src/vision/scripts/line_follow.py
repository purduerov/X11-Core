...
def draw_rect(img, contour):
  ...
  cv2.circle(img, (x + w/2, y + h/2), (255, 0, 0), -1)
  return [x+w/2,y+h/2]
...
def process(data):
  ...
  if contour.all() != -1:
    ...
    center_contour = draw_center(img_og, contour)
    cv2.line(img_og,(center_rect[0],center_rect[1]),(center_contour[0],center_contour[1]), (0,0,255),1.5)
  #find moment
